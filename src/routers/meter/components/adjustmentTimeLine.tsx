...
class AdjustmentChart extends React.Component<IProps, IState> {
    ...
    public handleStepData = (
        stepData: IStepData[],
        date: Moment,
        standardPriceItem: IStandardPriceRecord,
        timeIndex: number
    ) => {
        let result: ITimeItem[] = [];
        stepData.map((item, index) => {
            ...
            const seriesName = isFirst
                ? `≤ ${item.max}${unitTransfer(standardPriceItem.unit)}`
                : isEnd
                ? `＞${item.min}${unitTransfer(standardPriceItem.unit)}`
                : `${item.min}~${item.max}${unitTransfer(standardPriceItem.unit)}`;
            result.push({
