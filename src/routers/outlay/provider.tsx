...
export function isOutlayListRecordCanChecked(record: OutLayListItem, selectedRows: OutLayListItem[]) {
    ...
    */
    ...
    for(let i = 0, len = selectedRows.length; i < len; i++) {
        ...
        if(feeItems[0]?.can_receipt === 0 && feeItems[0]?.can_invoicing === 1 && fee_items[0]?.receipt_status === '开票中' && fee_items[0]?.can_invoicing === 0) {
        ...
        if(feeItems[0]?.can_receipt === 0 && feeItems[0]?.can_invoicing === 0 && fee_items[0]?.receipt_status === '开票中') {
            // 后端数据有点乱（开票中的但是确实已经开票了），先在前端解决，后续优化
            if(fee_items[0]?.receipt?.length > 0) {
                return true;
            }
        ...
        if(feeItems[0]?.receipt_status === '开票中' && feeItems[0]?.can_invoicing === 1 && fee_items[0]?.can_invoicing === 0) {
            return false;
        }
    }
